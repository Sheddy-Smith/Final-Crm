...
import { Trash2, Save, Search, Filter, TrendingUp, TrendingDown, DollarSign, Users, Wrench } from "lucide-react";
...
const LabourLedgerTab = () => {
  ...
  const syncFromJobSheet = () => {
    ...
    toast.success("Labour Ledger synced from Job Sheet!");
  };
  ...
  const uniqueLabour = [...new Set(ledger.map(item => item.notes || 'Unknown'))];
  ...
  const filteredLedger = ledger.filter(item => {
    ...
    const matchesLabour = !selectedLabour || item.notes === selectedLabour;
    return matchesSearch && matchesPayment && matchesLabour;
  });

  const getLabourSummary = (labourName) => {
    const labourItems = ledger.filter(item => item.notes === labourName);
    const totalAmount = labourItems.reduce((sum, item) => sum + calculateTotal(item), 0);
    const paidItems = labourItems.filter(item => item.paymentStatus === "Yes").length;
    const unpaidItems = labourItems.filter(item => item.paymentStatus === "No").length;
    return { totalAmount, paidItems, unpaidItems, totalItems: labourItems.length };
  };

  const totalVoucherAmount = LabourData.reduce((sum, v) => sum + Number(v.amount || 0), 0);

  const globalStats = {
    totalLabour: uniqueLabour.length,
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-900 dark:to-gray-800 p-6">
      <div className="max-w-7xl mx-auto space-y-6">
        <div className="flex justify-between items-center">
          <div>
            <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Labour Ledger</h1>
            <p className="text-gray-500 dark:text-gray-400 mt-1">Manage labour accounts and payments</p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <Card className="p-4 bg-gradient-to-br from-orange-500 to-orange-600 text-white">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-orange-100 text-sm font-medium">Total Labour</p>
                <p className="text-3xl font-bold mt-1">{globalStats.totalLabour}</p>
              </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <Card className="lg:col-span-1 p-6">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-xl font-bold text-gray-900 dark:text-white">Labour</h2>
              <Button
                onClick={() => setSelectedLabour(null)}
                variant="outline"
                size="sm"
                className={!selectedLabour ? 'bg-blue-50 dark:bg-blue-900' : ''}
              >
                All
              </Button>
            </div>
            ...
            <div className="space-y-2 max-h-[600px] overflow-y-auto">
              {uniqueLabour.map((labourName, idx) => {
                const summary = getLabourSummary(labourName);
                const isSelected = selectedLabour === labourName;

                return (
                  <div
                    key={idx}
                    onClick={() => setSelectedLabour(labourName)}
                  ...
                  >
                    <h3 className="font-semibold text-gray-900 dark:text-white mb-2">{labourName}</h3>
                    <div className="grid grid-cols-2 gap-2 text-sm">
                      <div>
                        <p className="text-gray-500 dark:text-gray-400">Total</p>
                        <p className="font-semibold text-orange-600">₹{summary.totalAmount.toFixed(2)}</p>
                      </div>
                      <div>
          ...
          <Card className="lg:col-span-2 p-6">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-xl font-bold text-gray-900 dark:text-white">
                {selectedLabour ? `${selectedLabour}'s Work` : 'All Labour Work'}
              </h2>
            ...
            <div className="overflow-x-auto">
              ...
              {filteredLedger.length === 0 && (
                <div className="text-center py-12 text-gray-500 dark:text-gray-400">
                  <p className="text-lg font-medium">No labour work items found</p>
                  <p className="text-sm">Sync from job sheet to add labour items</p>
                </div>
              )}
            </div>
        ...
        <Card className="p-6">
          <h3 className="text-xl font-bold mb-4">Labour Voucher Data</h3>
          <div className="overflow-x-auto">
            <table className="min-w-full">
              <thead className="bg-gray-100 dark:bg-gray-700">
                <tr>
                ...
                {LabourData.length === 0 && (
                  <tr>
                    <td colSpan="6" className="text-center py-12 text-gray-500 dark:text-gray-400">
                      <p className="text-lg font-medium">No labour vouchers found</p>
                    </td>
                  </tr>
                )}
              </tbody>
          ...
          <div className="mt-4 text-right font-semibold space-y-1">
            <div className="text-xl text-blue-600">
              Voucher Total: ₹{totalVoucherAmount.toFixed(2)}
            </div>
          </div>
        </Card>
      </div>
